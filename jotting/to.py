...
def outlet(handler):
    ...
    ----------
    handler : callable
        A function of the form ``(log, *args, **kwargs)``, where ``log``
        is a log generated by a use, and ``*args, **kwargs`` are the parameters
        that initialize the outlet (e.g. `File(path=/path/to/file)`).
    """
    wrapping = wraps(handler)
